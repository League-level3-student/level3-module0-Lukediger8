...
public class WorldPanel extends JPanel implements MouseListener, ActionListener {
	...
	public void step() {
		...
		for(int i = 0; i < cellsPerRow; i++) {
	     	   ...
	     	   getLivingNeighbors(array2D, i, j);
	     	   //livingNeighbors[i][j] = getLivingNeighbors(array2D, i, j);
	     	   System.out.println(getLivingNeighbors(array2D,i,j));
	     	   }
	     	   }
		// 8. check if each cell should live or die
		for(int i = 0; i < cellsPerRow; i++) {
	     	   for(int j = 0; j < cellsPerRow;j++) {
	     	   array2D[i][j].liveOrDie(getLivingNeighbors(array2D,i,j));;
	     	   }
	     	   }
		repaint();				
	}
	...
	public void mousePressed(MouseEvent e) {
		...
		for(int i = 0; i < cellsPerRow; i++) {
	     	   for(int j = 0; j < cellsPerRow;j++) {
	     		   Cell temp = array2D[i][j];
	     		   if(e.getX() >= temp.getX() && e.getY() >= temp.getY() && e.getX() <= (temp.getX()+ cellSize)&& e.getY() <=(temp.getY()+ cellSize)) {
